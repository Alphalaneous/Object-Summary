...
bool ObjectList::init(std::map<int, int> objects, SortOptions sortOptions, CCSize const& size){
    ...
    std::vector<int> portalObjects = { 10, 11, 12, 13, 45, 46, 47, 99, 101, 111, 200, 201, 202, 203, 286, 287, 660, 745, 747, 749, 1331, 1334, 1933, 2064, 2902, 2926 };
    std::vector<int> padOrbObjects = { 35, 36, 67, 84, 140, 141, 1022, 1330, 1332, 1333, 1594, 1704, 1751, 3004, 3005, 3027 };
    std::vector<int> triggerObjects = { 22, 23, 24, 25, 26, 27, 28, 29, 30, 32, 33, 55, 56, 57, 58, 59, 104, 105, 744, 899, 900, 901, 1006, 1007, 1049, 1268, 
    ...
    std::vector<int> solidObjects = { 1, 2, 3, 4, 6, 7, 40, 62, 63, 64, 65, 66, 68, 69, 70, 71, 72, 74, 75, 76, 
    ...
    std::vector<int> hazardObjects = { 8, 9, 39, 61, 88, 89, 98, 103, 135, 144, 145, 177, 178, 179, 183, 184, 185, 186, 187, 188, 
    ...
    for (auto const & [k, v] : pairs){
        ...
        if(sortOptions.isTriggers && std::find(triggerObjects.begin(), triggerObjects.end(), k) == triggerObjects.end()) continue;
        if(sortOptions.isSolids && std::find(solidObjects.begin(), solidObjects.end(), k) == solidObjects.end()) continue;
        if(sortOptions.isHazards && std::find(hazardObjects.begin(), hazardObjects.end(), k) == hazardObjects.end()) continue;
